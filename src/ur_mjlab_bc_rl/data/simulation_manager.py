...
from ..utils.config_loader import SceneConfig
...
class SimulationManager:
    ...
    def __init__(
        ...
        *,
        ...
        # ── 子组件 ─────────────────────────────────────
        self._mj = MujocoWrapper(config.task.scene_file, render=render)
        if render:
        ...
        # ── 验证：记录频率不能超过相机渲染频率 ──
        _max_record = 1.0 / dataset_cfg.frame_interval_s if dataset_cfg.frame_interval_s > 0 else float('inf')
        for c in config.cameras:
            if c.fps < _max_record:
                import logging
                _log = logging.getLogger(__name__)
                _log.warning(
                    "相机 %s 渲染频率 (%d Hz) 低于记录频率 (%.0f Hz)，"
                    "将出现重复帧。建议将 recode_hz 设为 ≤ 相机 fps。",
                    c.name, c.fps, _max_record,
                )
        ...
        # ── 相机独立 FPS 调度 ──
        # 每相机的渲染间隔（秒），来自场景配置
        self._cam_dts: dict[str, float] = {
            c.name: c.dt for c in config.cameras
        }
        # 相机上次渲染的仿真时间
        self._cam_timers: dict[str, float] = {c.name: 0.0 for c in config.cameras}
    ...
    def run_episode(
        ...
        *,
        ...
        while t_sim < t_max:
            ...
            # ── 相机渲染（各自独立 FPS）──
            for cam_name, cam_dt in self._cam_dts.items():
                self._cam_timers[cam_name] += self._pdt
                if self._cam_timers[cam_name] >= cam_dt:
                    self._cam_timers[cam_name] -= cam_dt
                    self._collector.capture_camera(cam_name)
