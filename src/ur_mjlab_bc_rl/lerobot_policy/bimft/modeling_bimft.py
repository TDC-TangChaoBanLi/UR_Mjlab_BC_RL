...
def _build_policy_batch(
    ...
    # 在线评估时可能是 [B, D]，需要添加时序维度
    if state.ndim == 2:
        state = state.unsqueeze(1)  # [B, D] → [B, 1, D]
    # 可能形状:
    #   [B, inner_T, D]            — 无 n_obs_steps 堆叠 (单帧)
    #   [B, T_obs, R, D]           — 高频状态 delta 堆叠后 (env 返回 (R,D) 格式)
    #   [B, T_obs, D]              — 低频状态 delta 堆叠后 (env 返回 (D,) 格式)
    #   [B, T_obs, 1, R, D]        — delta indices + 维度展开

    # 规范化: squeeze 多余的 1 维（不 squeeze batch dim），确保最终是 [B, ...]
    # 可能的形状: [B, T_obs, 1, R, D] → [B, T_obs, R, D]
    if state.ndim == 5 and state.shape[2] == 1:
        state = state.squeeze(2)

    actual_state_T = state.shape[1]  # 用于后续 wrench fallback

    # 现在处理不同形状
    if state.ndim == 3:
        # [B, T_actual, D] → 对齐到 T，然后展开为 [B, T, R, D]
        if actual_state_T < T:
            pad = torch.zeros(B, T - actual_state_T, state.shape[-1], device=device, dtype=state.dtype)
            state = torch.cat([state, pad], dim=1)
        elif actual_state_T > T:
            state = state[:, :T, :]
        # 扩展为 [B, T, R, D]（低频状态重复 R 次）
        state = state.unsqueeze(2).expand(B, T, R, state.shape[-1])
    elif state.ndim == 4:
        # [B, T_obs, R_actual, D] — 高频状态 delta 堆叠后
        state = state[:, :T, :, :]  # 截断到 T
        R_actual = state.shape[2]
        if R_actual < R:
            # 填充 R 维度: [B, T, R_actual, D] → [B, T, R, D]
            pad = torch.zeros(B, T, R - R_actual, state.shape[-1],
    ...
    def _build_wrench(f_key: str | None, t_key: str | None, start_idx: int) -> torch.Tensor:
        """组合 force(3) + torque(3) → wrench(6)，对齐到 [B, T, R, 6]。

        支持格式:
          - [B, D]               → 单帧低频 → [B, 1, R, D//2]
          - [B, T_w, D]          → 多帧低频 → [B, T_w, R, D//2]
          - [B, T_w, R_w, D]     → 多帧高频 (ndim=4) → [B, T_w, R_w, D//2]
        """
        force = batch.get(f_key) if f_key else None
        torque = batch.get(t_key) if t_key else None

        # 在线评估时可能缺少时序维度: [B, D] → [B, 1, D]
        if force is not None and force.ndim == 2:
            force = force.unsqueeze(1)
        if torque is not None and torque.ndim == 2:
        ...
        if force is not None and torque is not None:
            ...
            wrench = torch.cat([f_arm, t_arm], dim=-1)
        elif force is not None:
            f_dim = force.shape[-1] // 2
            f_arm = force[..., start_idx * f_dim:(start_idx + 1) * f_dim]
            wrench = torch.cat([f_arm, torch.zeros_like(f_arm)], dim=-1)
        else:
            return torch.zeros(B, T, R, 6, device=device)

        # 对齐 R 维度
        if wrench.ndim == 3:
            # [B, T_w, 6] → [B, T_w, R, 6]
            wrench = wrench.unsqueeze(2).expand(-1, -1, R, -1)
        elif wrench.ndim == 4:
            R_w = wrench.shape[2]
            if R_w < R:
                pad = torch.zeros(wrench.shape[0], wrench.shape[1], R - R_w, wrench.shape[-1],
                                  device=device, dtype=wrench.dtype)
                wrench = torch.cat([wrench, pad], dim=2)
            elif R_w > R:
                wrench = wrench[:, :, :R, :]
        return wrench
...
class BiMFTPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # 取动作块的第一步 → 拼接左右 → 确保 float32
        left = out["left_action"][:, 0, :]   # [B, D_a]
        right = out["right_action"][:, 0, :] # [B, D_a]
        return torch.cat([left, right], dim=-1).float()
